...
print("=" * 60)
print("🚀 启动消融实验: R18-Fixed (ResNet18 + 常规阈值)")
print("✨ 已加入 Image-level & Pixel-level 双重 F1 精确计算")
print("=" * 60)
...
# ✅ 成绩单(CSV) 专属落盘路径
SAVE_DIR = Path(r"D:\code\industrial\results\visa_patchcore_wrn50\ALL_RUN_R18_FIXED")
CSV_RESULT_PATH = SAVE_DIR / "Auto_R18_Fixed_Metrics.csv"

# ✅ 4栏对比图 专属落盘路径
FIGURE_DIR = Path(r"D:\code\industrial\paper_figures\visa\ALL_RUN_R18_FIXED")

if not SAVE_DIR.exists(): SAVE_DIR.mkdir(parents=True)
if not FIGURE_DIR.exists(): FIGURE_DIR.mkdir(parents=True)
...
def run_single_category(category, PatchcoreCls, EngineCls):
    ...
    # 🚨 将 Backbone 切换为 resnet18
    model = PatchcoreCls(backbone="resnet18", coreset_sampling_ratio=0.1)
    ...
    # 🚀 手动算出绝对真实的 Image-level 和 Pixel-level 指标
    all_amaps = []
    all_masks = []

    # 🌟 新增：用于存储每张图的最大得分和真实标签 (Image-level)
    all_img_scores = []
    all_img_labels = []
    ...
    for batch in predictions:
        ...
        # 提取 Image-level 得分：通常取 anomaly_map 的最大值作为整图的异常分数
        for i in range(len(batch.label)):
            img_score = batch.anomaly_map[i].max().cpu().item()
            all_img_scores.append(img_score)
            all_img_labels.append(batch.label[i].cpu().item())
    ...
    if len(all_img_labels) > 0:
        # ==========================================
        # 📊 1. 计算 Image-level (打擂台专用) 分数
        # ==========================================
        try:
            img_auroc = roc_auc_score(all_img_labels, all_img_scores)
        except Exception as e:
            print(f"Image AUROC 计算失败: {e}")

        img_precision, img_recall, img_thresholds = precision_recall_curve(all_img_labels, all_img_scores)
        img_f1_scores = 2 * (img_precision * img_recall) / (img_precision + img_recall + 1e-8)
        best_img_idx = np.argmax(img_f1_scores)
        best_img_f1 = img_f1_scores[best_img_idx]

        # ==========================================
        # 📊 2. 计算 Pixel-level (严苛的定位) 分数
        # ==========================================
        if len(all_masks) > 0:
            ...
            pixel_precision, pixel_recall, pixel_thresholds = precision_recall_curve(flat_masks, flat_amaps)
            pixel_f1_scores = 2 * (pixel_precision * pixel_recall) / (pixel_precision + pixel_recall + 1e-8)
            best_pixel_idx = np.argmax(pixel_f1_scores)
            best_pixel_threshold = pixel_thresholds[best_pixel_idx]
            best_pixel_f1 = pixel_f1_scores[best_pixel_idx]
        ...
        with open(CSV_RESULT_PATH, mode='a', newline='', encoding='utf-8-sig') as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow(["Category (类别)", "Image_AUROC", "Image_F1Score", "Pixel_AUROC", "Pixel_F1Score"])
            writer.writerow(
                [category, f"{img_auroc:.4f}", f"{best_img_f1:.4f}", f"{pixel_auroc:.4f}", f"{best_pixel_f1:.4f}"])

        print(f"✅ [{category}] 成绩落盘: Image F1={best_img_f1:.4f} | Pixel F1={best_pixel_f1:.4f}")
    ...
    for batch in predictions:
        for i in range(len(batch.label)):
            ...
            # 画图时使用找到的 best_pixel_threshold 才能最精准地扣出红圈
            mask_bool = amap_np > best_pixel_threshold
...
def main():
    ...
    print(f"\n🎉 恭喜！R18-Fixed 双重指标计算运行完毕。")
    print(f"📊 最终包含 Image & Pixel F1 的成绩单保存在: {CSV_RESULT_PATH}")
    print(f"🎨 所有4栏可视化图保存在: {FIGURE_DIR}")
